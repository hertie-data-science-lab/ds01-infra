...
if python3 tests/gpu-allocation/test-gpu-state-reader.py 2>&1 | grep -q "OK"; then
    echo -e "  ${GREEN}✓${NC} GPU State Reader tests passed"
else
    echo -e "  ${RED}✗${NC} GPU State Reader tests failed"
    python3 tests/gpu-allocation/test-gpu-state-reader.py
fi
echo ""

# Test 2: GPU Availability Checker
echo -e "${BLUE}[2/4]${NC} Testing gpu_availability_checker..."
if python3 tests/gpu-allocation/test-gpu-availability-checker.py 2>&1 | grep -q "OK"; then
    echo -e "  ${GREEN}✓${NC} GPU Availability Checker tests passed"
else
    echo -e "  ${RED}✗${NC} GPU Availability Checker tests failed"
    python3 tests/gpu-allocation/test-gpu-availability-checker.py
fi
echo ""

# Test 3: GPU Allocator Smart
echo -e "${BLUE}[3/4]${NC} Testing gpu_allocator_smart..."
if python3 tests/gpu-allocation/test-gpu-allocator-smart.py 2>&1 | grep -q "OK"; then
    echo -e "  ${GREEN}✓${NC} GPU Allocator Smart tests passed"
else
    echo -e "  ${RED}✗${NC} GPU Allocator Smart tests failed"
    python3 tests/gpu-allocation/test-gpu-allocator-smart.py
fi
echo ""

# Test 4: Integration Tests
echo -e "${BLUE}[4/4]${NC} Running integration tests..."
if bash tests/gpu-allocation/test-integration.sh 2>&1 | tail -1 | grep -q "All tests passed"; then
