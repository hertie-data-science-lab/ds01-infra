...
| Test-only lib `container-session.sh` | `scripts/lib/container-session.sh` | Referenced only by `testing/unit/lib/test_container_session.py`. Also under active edit on `refactor/unified-user-caps` (WIP). | Likely mid-wiring — finish integrating it, or remove lib + test if abandoned. |
