...
- **`get_user_home()` consolidation** (surfaced by `/simplify`): the `getent passwd "$x" | cut -d: -f6` home-lookup idiom now appears at 3+ sites (`mlc-create-wrapper.sh`, `bypass-enforce-containers.sh`, pre-existing `pam-ensure-bashrc.sh`; plus the deferred `enforce-containers.sh` eval would become a 4th). Worth extracting a `get_user_home()` helper into `scripts/lib/username-utils.sh` (already `getent`-based) so the not-found guard lives in one place. Deferred from the 2026-06 pass: `bypass` doesn't yet source that lib, one site is outside the touched diff, and the current guarding asymmetry is intentional (`mlc-create-wrapper`'s `SUDO_USER` is always present). Best done holistically with the `enforce-containers.sh` item above.
