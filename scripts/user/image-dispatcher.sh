...
show_usage() {
    ...
    echo -e "  ${GREEN}list (--detailed)${NC}       List available images"
    echo -e "  ${GREEN}update ${NC}                 Rebuild/update an existing image"
    echo -e "  ${GREEN}delete (also remove) ${NC}   Remove unused images"
    echo ""
    echo -e "${BOLD}Examples:${NC}"
    echo -e "  ${CYAN}image create${NC}"
    echo -e "  ${CYAN}image list --detailed${NC}"
    echo -e "  ${CYAN}image update${NC} my-image"
    echo -e "  ${CYAN}image delete${NC} old-image"
    echo -e "  ${CYAN}image remove${NC}"
    echo ""
...
if [ ! -f "$SUBCOMMAND_SCRIPT" ]; then
    ...
    echo ""
    echo "Available subcommands: create, list, update, delete/remove"
