        ...
        try:
            result = subprocess.run(
                [
                    ...
                    "ps",
                    "--filter",
                    "label=ds01.gpu.slots",
                    "--format",
                    '{{.Names}}|{{.Label "ds01.user"}}|{{.Label "ds01.gpu.slots"}}|{{.Label "ds01.gpu.uuids"}}',
                ],
                capture_output=True,
                text=True,
                timeout=10,
            ...
            for line in result.stdout.strip().split("\n"):
                ...
                if len(parts) >= 3:
                    ...
                    gpu_uuids = [
                        u.strip()
                        for u in (parts[3] if len(parts) > 3 else "").split(",")
                        if u.strip()
                    ]
            ...
            print(
                f"Error getting allocations (both methods failed): {e}, {fallback_error}",
                file=sys.stderr,
            )
            return []
...
def check_wasted_allocations():
    ...
    if not UTILIZATION_LOG.exists():
        ...
        print("Run 'sudo mig-utilization-monitor.py --record' periodically to collect data.")
        return []
    ...
    if len(history) < 3:
        ...
        print("Run 'sudo mig-utilization-monitor.py --record' every 5 minutes to collect data.")
        return []
...
def format_utilization_display(mig_instances, allocations):
    ...
    for instance in mig_instances:
        gpu = instance["gpu"]
        if gpu not in gpus:
            gpus[gpu] = {"name": instance["gpu_name"], "instances": []}
    ...
    for gpu_idx in sorted(gpus.keys()):
        ...
        for instance in gpu_info["instances"]:
            ...
            lines.append(
                f"  MIG {slot} [{profile}]: {color}{util:3d}%{reset} | Mem: {process_mem:5d}MB | {status}{container_info}"
            )
    ...
    lines.append("-" * 70)
    lines.append(
        f"Total MIG Instances: {total_instances} | Allocated: {allocated} | Free: {free} | Avg Util (allocated): {avg_util:.1f}%"
    )
...
def main():
    ...
    parser.add_argument(
        "--check-waste", action="store_true", help="Check for wasted MIG allocations"
    )
    ...
    if args.check_waste:
        ...
        if wasted:
            ...
            for w in wasted:
                print(
                    f"  - {w['container']} ({w['user']}): MIG {w['mig_slot']} - {w['waste_ratio'] * 100:.0f}% idle"
                )
                log_event(
                    "alert.mig_waste",
                    w["user"],
                    f"MIG {w['mig_slot']} underutilized in {w['container']}",
                )
        else:
