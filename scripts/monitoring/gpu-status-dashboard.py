...
class GPUStatusDashboard:
    ...
    def generate_markdown(self) -> str:
        ...
        # Footer
        md += "---\n"
        md += "*Auto-generated by DS01 GPU Status Dashboard*\n"
        md += "*Allocation is dynamic: GPUs assigned on container start, released on stop*\n"
    ...
    def print_terminal(self):
        ...
        for gpu_id in sorted(state["gpus"].keys(), key=int):
            ...
            print(
                f"  Memory: {gpu_hw.get('memory_used', '?')}/{gpu_hw.get('memory_total', '?')} MB"
            )
