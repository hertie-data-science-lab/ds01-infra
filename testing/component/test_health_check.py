...
class TestHealthCheckScript:
    ...
    def test_help_flag(self):
        ...
        result = subprocess.run(
            [str(self.HEALTH_CHECK), "--help"], capture_output=True, text=True, timeout=10
        )
    ...
    def test_basic_execution(self):
        ...
        result = subprocess.run(
            [str(self.HEALTH_CHECK)], capture_output=True, text=True, timeout=60
        )
...
class TestHealthCheckOutput:
    ...
    def test_output_has_status_indicators(self):
        ...
        has_indicators = any(
            ind in output for ind in ["PASS", "FAIL", "OK", "ERROR", "WARN", "✓", "✗"]
        )
    ...
    def test_json_output_option(self):
        ...
        result = subprocess.run(
            [str(self.HEALTH_CHECK), "--json"], capture_output=True, text=True, timeout=60
        )
