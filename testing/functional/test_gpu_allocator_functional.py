...
def test_gpu_allocation_workflow():
    ...
    manager = GPUAllocationManager(state_dir=f"{test_dir}/state", log_dir=f"{test_dir}/logs")  # noqa: F821
    ...
    gpu1, status1 = manager.allocate_gpu(
        "student1", "student-container._.1001", max_gpus=1, priority=10
    )
    ...
    assert student_count_after == 0, (
        f"Expected student1 to have 0 GPUs after release, got {student_count_after}"
    )
