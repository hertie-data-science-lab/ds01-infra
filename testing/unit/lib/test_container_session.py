...
class TestContainerSessionScript:
    ...
    def run_bash_script(
        ...
        return subprocess.run(
            ["bash", "-c", script_content], capture_output=True, text=True, env=run_env, timeout=10
        )
...
class TestValidateGpuAvailableLive:
    ...
    def test_validate_gpu_handles_empty_response_gracefully(self):
        ...
        assert (
            '[ -z "$gpu_uuids" ]' in content
            or '[[ -z "$gpu_uuids"' in content
            or '-z "$gpu_uuids"' in content
            or "<no value>" in content
        )
...
class TestErrorMessages:
    ...
    def test_error_message_includes_uuid(self):
        ...
        assert (
            "GPU $gpu_uuid is no longer available" in content
            or "no longer available" in content.lower()
        )
